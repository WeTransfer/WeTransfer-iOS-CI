...
public enum WeTransferPRLinter {
    public static func lint(
        ...
        measure(taskName: "SwiftLint") {
            swiftLint(
                using: danger,
                executor: swiftLintExecutor,
                configsFolderPath: swiftLintConfigsFolderPath,
                fileManager: fileManager
            )
        }
    }
    ...
    static func reportXCResultsSummary(
        ...
        do {
            ...
            summaryReporter
                .reportXCResultSummary(for: xcResultFiles, using: danger, fileManager: fileManager) { result in
    ...
    static func showBitriseBuildURL(
        using danger: DangerDSL,
        environmentVariables: [String: String] = ProcessInfo.processInfo.environment
    ) {
    ...
    static func swiftLint(
        ...
        let configsFolderPath: String = {
            if let configsFolderPath = configsFolderPath, fileManager.fileExists(
                atPath: configsFolderPath,
                isDirectory: nil
            ) {
...
extension WeTransferPRLinter {
    ...
    static func validateMarkUsage(
        using danger: DangerDSL,
        file: Danger.File,
        lines: [String],
        minimumLinesCount: Int = 200
    ) {
