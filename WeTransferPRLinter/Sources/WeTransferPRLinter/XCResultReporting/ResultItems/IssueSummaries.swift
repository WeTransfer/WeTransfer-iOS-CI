...
extension ResultIssueSummaries {
    func createResults(
        ...
        results.append(
            contentsOf: testFailureSummaries
    ...
    func createResults(
        ...
        let failedAndRetryResults = compactMap { testFailureIssueSummary -> [XCResultItem]? in
            ...
            if retriedTestIdentifiers.contains(identifier) {
                return testFailureIssueSummary.createTestRetriedResult(
                    context: context,
                    testPlanRunSummaries: testPlanRunSummaries
                )
            } else if failedTestIdentifiers.contains(identifier) {
                return testFailureIssueSummary.createTestFailureResult(
                    context: context,
                    testPlanRunSummaries: testPlanRunSummaries
                )
            }
        ...
        let skippedResults: [XCResultItem] = testPlanRunSummaries.skippedTests
            .compactMap { actionTestMetadata -> [XCResultItem]? in
...
extension TestFailureIssueSummary {
    func createTestFailureResult(
        ...
        return [XCResultItem(
            ...
            line: fileMetadata?.line,
            category: .error
        )]
    }

    func createTestRetriedResult(
        context: ResultGenerationContext,
        testPlanRunSummaries: ActionTestPlanRunSummaries
    ) -> [XCResultItem] {
